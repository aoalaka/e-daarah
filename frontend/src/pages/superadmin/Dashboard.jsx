...
function SuperAdminDashboard() {
  ...
  const [visitorAnalyticsDays, setVisitorAnalyticsDays] = useState(7);
  const [visitorAnalyticsLoading, setVisitorAnalyticsLoading] = useState(false);
  const [visitorAnalyticsError, setVisitorAnalyticsError] = useState(null);
  ...
  const fetchVisitorAnalytics = async (days = 7) => {
    setVisitorAnalyticsLoading(true);
    setVisitorAnalyticsError(null);
    try {
      const response = await api.get(`/superadmin/visitor-analytics?days=${days}`, getAuthHeader());
      setVisitorAnalytics(response.data);
    } catch (error) {
      const msg = error.response?.data?.error || 'Failed to load visitor analytics';
      setVisitorAnalyticsError(msg);
...
<table role="presentation" style="width:100%;border-collapse:collapse;margin-top:32px;border-top:1px solid #eee;padding-top:24px;"><tr><td style="padding-top:24px;">
...
<p style="margin:8px 0 0 0;font-size:12px;color:#aaa;">Madrasah Administration Made Simple</p>
  ...
  const handleSendBroadcast = async () => {
    ...
    try {
      ...
      let msg = `Sent: ${sent}, Failed: ${failed}, Total: ${total}`;
  ...
  const fetchSmsCredits = async () => {
    try {
      const response = await api.get('/superadmin/sms-credits', getAuthHeader());
      setSmsCredits(response.data.madrasahs || []);
    } catch (error) {
  ...
  const handleGrantCredits = async (e) => {
    e.preventDefault();
    if (!grantForm.madrasah_id || !grantForm.credits) return;
    setGrantLoading(true);
    try {
      const res = await api.post('/superadmin/sms-credits/grant', {
        madrasah_id: parseInt(grantForm.madrasah_id),
        credits: parseInt(grantForm.credits),
        note: grantForm.note
      }, getAuthHeader());
      alert(`Granted ${res.data.creditsGranted} credits to ${res.data.madrasah}. New balance: ${res.data.newBalance}`);
      setGrantForm({ madrasah_id: '', credits: '', note: '' });
      setShowGrantForm(false);
      fetchSmsCredits();
      fetchRevenue();
    } catch (error) {
      alert(error.response?.data?.error || 'Failed to grant credits');
    } finally {
  ...
  const handleTestSms = async (e) => {
    e.preventDefault();
    if (!testSmsForm.phone || !testSmsForm.message) return;
    setTestSmsLoading(true);
    setTestSmsResult(null);
    try {
      const res = await api.post('/superadmin/sms-test', testSmsForm, getAuthHeader());
      setTestSmsResult({ success: true, sid: res.data.sid, status: res.data.status });
    } catch (error) {
      setTestSmsResult({ success: false, error: error.response?.data?.error || 'Failed to send' });
    } finally {
          ...
          <section className="stats-grid">
            ...
            {stats.smsRevenue > 0 && (
              <div className="stat-card" onClick={() => setActiveTab('revenue')} style={{ cursor: 'pointer' }}>
                <div className="stat-value">${stats.smsRevenue}</div>
                <div className="stat-label">SMS Revenue</div>
              </div>
            )}
          </section>
        )}
